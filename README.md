...
- **创意讨论群（默认）**：服务端按固定最多 7 轮调度七个 Agent，`@` 用于观点协作与主持选人
...
默认“创意讨论群”由七个角色组成：创想家 A（视觉/IP，`creative`）、创想家 B（产品/玩法，`brand`）、创想家 C（内容/社媒，`product`）、创想家 D（跨界/联名/体验，`content`）、市场现实校准员（`market`）、讨论主持人（`moderator`）和创意总监（`director`）。流程固定最多 7 轮；A/B/C/D 在发散、交叉回应和快速修正轮真正并行，每个人设仅保留约 6–10 行核心规则。

模型策略保存在 Agent 配置中：A/B/C/D 与讨论主持人使用 `gpt-5.6-luna` + `low`，市场现实校准员使用 `gpt-5.6-terra` + `low`，创意总监使用 `gpt-5.6-terra` + `medium`。未配置的普通 Agent 不传模型覆盖，继续使用 Codex 当前默认模型。

默认群的一条用户主题会启动一次服务端控制的讨论，严格最多 7 轮：四路并行发散 → 交叉回应 → 主持挑选争议并自由讨论 → 市场校准 → 四路快速修正 → 市场二次筛选 → 创意总监输出 TOP3–5。Agent 的 `@` 只用于支持、反驳、组合、延伸和主持选人，不会触发无限调用。每次只注入原始主题、最多 3 条讨论摘要和上一轮最多 4 条必要消息，并逐 Agent 持久化 `queue_wait_ms`、`codex_execution_ms`、`total_ms` 与 `prompt_tokens`。普通自建群仍保留自由 `@` 协作和私聊。
